...
import com.jeremyseq.clashlings.client.Camera;
import com.jeremyseq.clashlings.client.LevelRenderer;
import com.jeremyseq.clashlings.common.level.*;
...
public class LevelEditor extends JPanel implements ActionListener, KeyListener, Camera {
    ...
    public void draw(Graphics g, ImageObserver imageObserver) {
        if (this.getMousePosition() != null) {
            ...
            Vec2 worldPos = this.getWorldPositionFromRenderPosition(mousePos);
            Vec2 tilePos = getTilePositionFromWorldPosition(worldPos);
            Vec2 startPos = new Vec2(tilePos.x * LevelRenderer.DRAW_SIZE, tilePos.y * LevelRenderer.DRAW_SIZE);
            startPos = this.getRenderPositionFromWorldPosition(startPos);
            g.drawRect((int) startPos.x, (int) startPos.y, LevelRenderer.DRAW_SIZE, LevelRenderer.DRAW_SIZE);
        ...
        g.drawString("Layer: " + layer, LevelRenderer.DRAW_SIZE + 10, (int) (bounds.getHeight()+2));
    }
    ...
    public Vec2 getTilePositionFromWorldPosition(Vec2 worldPos) {
        // convert world position to tile position
        Vec2 tilePos = new Vec2(worldPos.x / LevelRenderer.DRAW_SIZE, worldPos.y / LevelRenderer.DRAW_SIZE);
